...
class CostBreaker:
    ...
    def _reset_daily_stats(self, stats: UsageStats, api_key: str = ""):
        ...
        if now.date() > stats.last_reset.date():
            ...
            if api_key:
                logger.info("Daily usage stats reset", api_key_hash=str(hash(api_key))[:8])
    ...
    def check_daily_spend_limit(self, api_key: str, estimated_cost: float = 0.0) -> Tuple[bool, str]:
        ...
        self._reset_daily_stats(stats, api_key)
    ...
    def check_anomaly_detection(self, api_key: str, current_rps: float, current_spend: float) -> Tuple[bool, str]:
        ...
        if stats.last_anomaly and (datetime.now() - stats.last_anomaly).seconds < self.anomaly_cooldown:
            remaining = self.anomaly_cooldown - (datetime.now() - stats.last_anomaly).seconds
            return False, f"Anomaly cooldown active: {remaining}s remaining"
