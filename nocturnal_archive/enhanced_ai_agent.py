...
class EnhancedNocturnalAgent:
    ...
    def __init__(self):
        ...
        try:
            self.per_user_token_limit = int(os.getenv("GROQ_PER_USER_TOKENS", 25 * 850))
        except (TypeError, ValueError):
            self.per_user_token_limit = 25 * 850
    ...
    def _build_system_prompt(
        ...
        sections: List[str] = []
        sections.append(
            "You are the Enhanced Nocturnal AI Agent, a concise and trustworthy research assistant with direct access to production-grade data sources."
        )
        ...
        if "shell" in apis:
            capability_lines.append("• Persistent shell session for lightweight system inspection")
        if not capability_lines:
            capability_lines.append("• Core reasoning, code generation, and memory recall")
        sections.append("Capabilities in play:\n" + "\n".join(capability_lines))

        rules = [
            "Be direct and cite statistics or papers using the links from the API results.",
            "If an API payload includes errors or missing data, say so—do not fabricate numbers.",
            "Never invent financial figures or research facts beyond the supplied results.",
            "Keep responses lean (no <think> tags or filler).",
            "Only surface shell commands when the user asks about files or system state.",
            "Quote relevant lines from provided file previews and reference their paths."
        ]
    ...
    async def initialize(self, force_reload: bool = False):
        ...
        async with lock:
            ...
            # Try multiple API keys for better rate limits (supports up to 16 keys)
            api_keys: List[str] = []
            for idx in range(1, 17):
                env_name = 'GROQ_API_KEY' if idx == 1 else f'GROQ_API_KEY_{idx}'
            ...
            if self.api_keys:
                for idx, key in enumerate(self.api_keys):
                    try:
                        ...
                        self.current_api_key = key
                        print(f"✅ Enhanced Nocturnal Agent Ready! (Using API key {idx + 1})")
                        break
                    except Exception as e:
            ...
            if not self.client:
                if not self.api_keys:
                    print("⚠️ No Groq API keys configured. Agent will operate in fallback mode until a key is added.")
                else:
                    print("❌ Unable to initialize Groq client with provided keys. Fallback mode only.")
